...
execute store result score #test value run execute if entity @e[tag=w,type=armor_stand,tag=BAIautocount,limit=1]
execute if score #test value matches ..0 run tag @e[tag=w,type=armor_stand] add BAIautocount
execute if score #test value matches 1.. run tag @e[tag=w,type=armor_stand] remove BAIautocount

#sign
execute if entity @e[tag=w,type=armor_stand,tag=BAIautocount] run data merge block 1613 88 355 {front_text:{has_glowing_text:0b,messages:['{"text":"","clickEvent":{"action":"run_command","value":"function sprint_racer:game_logic/0/options_signs/ai_add_type_battle"}}','{"text":"PLAYER COUNT","bold":true,"color":"black"}','{"text":"[MEET]","color":"dark_blue"}','{"text":""}']},is_waxed:1b}
