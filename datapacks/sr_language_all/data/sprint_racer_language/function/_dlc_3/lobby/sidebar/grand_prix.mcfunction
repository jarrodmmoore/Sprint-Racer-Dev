...
execute unless entity @e[type=armor_stand,tag=w,x=1560,y=150,z=406,distance=..1,tag=endlessMode] run scoreboard objectives add lobbyDisplay dummy ["",{translate:"sr.grand_prix",color:"gold",bold:true}]
execute if entity @e[type=armor_stand,tag=w,x=1560,y=150,z=406,distance=..1,tag=endlessMode] run scoreboard objectives add lobbyDisplay dummy ["",{translate:"sr.lobby.endless_mode",color:"gold",bold:true}]
