...
#SLOT 6
item replace entity @s[gamemode=creative,tag=!restricted,tag=!selecting,tag=!nearChest,scores={playerState=2}] hotbar.6 with minecraft:coal[custom_name=["",{text:"RESERVED",color:"dark_gray",bold:true,italic:false}]] 1
item replace entity @s[gamemode=creative,tag=!restricted,tag=!selecting,tag=nearChest,scores={playerState=2}] hotbar.6 with minecraft:orange_dye[custom_name=["",{keybind:"key.drop",color:"yellow",bold:true,italic:false},{text:" >> Edit Item Chest",color:"gold",bold:true,italic:false}]] 1
