...
execute if entity @e[tag=w,tag=realms] run tellraw @a[tag=!minChat] ["",{"translate":"sr.lobby.options.battle_ai_count","color":"green"},{"score":{"name":"@e[tag=w,limit=1]","objective":"optBAIcount"},"color":"green"}]
execute if entity @e[tag=w,tag=!realms] run tellraw @a[tag=!minChat] ["",{"translate":"sr.lobby.options.battle_ai_count","color":"green"},{"score":{"name":"@e[tag=w,limit=1]","objective":"optBAIcount"},"color":"green"},{"text":" "},{"text":" "},{"text":"[!]","color":"red","bold":true,"hoverEvent":{"action":"show_text","contents":"Higher AI counts are very taxing on the\nserver! Make sure your server's tick length\nstays below 50ms during gameplay.\nAbove 50ms, the AI will slow down."}}]
