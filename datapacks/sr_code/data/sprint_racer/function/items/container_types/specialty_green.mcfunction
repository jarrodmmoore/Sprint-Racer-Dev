tag @e[limit=1,sort=nearest,tag=itemcontainer,tag=chest8,type=armor_stand,distance=..3] add chestTarget
...
execute as @e[tag=chestTarget,type=armor_stand,distance=..3] at @s run particle block{block_state:"minecraft:lime_wool"} ~ ~1.5 ~ .2 .2 .2 1 20
...
execute if entity @e[type=armor_stand,tag=cheats,x=1548,y=148,z=422,distance=..1,tag=15a] run function sprint_racer:items/pick_item/trap/_main
