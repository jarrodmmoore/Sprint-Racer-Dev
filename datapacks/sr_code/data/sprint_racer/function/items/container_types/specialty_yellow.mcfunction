tag @e[limit=1,sort=nearest,tag=itemcontainer,tag=chest6,type=armor_stand,distance=..3] add chestTarget
...
execute as @e[tag=chestTarget,type=armor_stand,distance=..3] at @s run particle block{block_state:"minecraft:yellow_wool"} ~ ~1.5 ~ .2 .2 .2 1 20
...
execute if entity @e[tag=cheats,x=1548,y=148,z=422,distance=..1,type=armor_stand,tag=15a] run function sprint_racer:items/pick_item/defense/_main
