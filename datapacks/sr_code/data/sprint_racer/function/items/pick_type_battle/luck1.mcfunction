...
execute if score #catRed value matches 0 run tag @e[tag=random,scores={rNumber=1..23}] add tempBan
execute if score #catYel value matches 0 run tag @e[tag=random,scores={rNumber=24..33}] add tempBan
execute if score #catBlu value matches 0 run tag @e[tag=random,scores={rNumber=34..41}] add tempBan
execute if score #catGre value matches 0 run tag @e[tag=random,scores={rNumber=42..50}] add tempBan
#execute if score #catPur value matches 0 run tag @e[tag=random,scores={rNumber=1..5}] add tempBan

#don't pick the same category twice in a row, please! (except offense. offense is fine)
execute if score @s lastCatPicked matches 2 if score #catTotal value matches 2.. run tag @e[tag=random,type=armor_stand,scores={rNumber=24..33}] add tempBan
execute if score @s lastCatPicked matches 3 if score #catTotal value matches 2.. run tag @e[tag=random,type=armor_stand,scores={rNumber=34..41}] add tempBan
execute if score @s lastCatPicked matches 4 if score #catTotal value matches 2.. run tag @e[tag=random,type=armor_stand,scores={rNumber=42..50}] add tempBan
