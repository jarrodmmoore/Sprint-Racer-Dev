...
execute if entity @s[tag=itemCyan] positioned ~-1 ~-1 ~-1 if entity @e[tag=activeplayer,tag=playerOrange,tag=!rocketOwner,dx=2,dy=2,dz=2] run scoreboard players set #hit value 1
execute if entity @s[tag=!itemCyan] positioned ~-1 ~-1 ~-1 if entity @e[tag=activeplayer,tag=playerCyan,tag=!rocketOwner,dx=2,dy=2,dz=2] run scoreboard players set #hit value 1
