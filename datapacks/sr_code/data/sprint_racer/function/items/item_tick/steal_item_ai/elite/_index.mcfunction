...
execute if entity @e[tag=stealTarget1,scores={aiHasCustomItem=1..}] run tag @e[tag=random,x=1548,y=155,z=406,distance=..1,scores={rNumber=22}] add igotthis

scoreboard players set @s rNumber 0
scoreboard players operation @s rNumber = @e[limit=1,sort=random,tag=random,x=1548,y=155,z=406,distance=..1,tag=igotthis,scores={rNumber=1..22}] rNumber
#DEBUG
...
execute if entity @s[tag=!gotEm,scores={rNumber=22}] run function sprint_racer:items/item_tick/steal_item_ai/custom with storage sprint_racer:func_args
