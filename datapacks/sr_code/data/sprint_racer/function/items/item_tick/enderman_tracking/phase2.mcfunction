...
tag @e[limit=1,sort=random,tag=playing,tag=!stealer,scores={invisibility=..0},gamemode=!spectator,nbt={SelectedItem:{components:{"minecraft:custom_data":{item:1b,eliteitem:1b}}}}] add stealTarget1
...
execute unless entity @e[tag=stealTarget1] run tag @e[limit=1,sort=random,tag=playing,tag=!stealer,scores={invisibility=..0},gamemode=!spectator,nbt={SelectedItem:{components:{"minecraft:custom_data":{item:1b,normalitem:1b}}}}] add stealTarget2
...
execute if entity @e[tag=ai] unless entity @e[tag=stealTarget1] unless entity @e[tag=stealTarget2] run function sprint_racer:items/item_tick/enderman_tracking/phase2_ai
