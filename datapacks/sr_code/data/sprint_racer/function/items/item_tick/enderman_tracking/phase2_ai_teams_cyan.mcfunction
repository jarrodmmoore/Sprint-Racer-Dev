...
tag @e[limit=1,sort=random,tag=ai,tag=!stealer,tag=playerOrange,scores={invisibility=..0,aiHasElite=1..}] add stealTarget1
...
execute unless entity @e[tag=stealTarget1] run tag @e[limit=1,sort=random,tag=ai,tag=playerOrange,tag=!stealer,scores={invisibility=..0,aiHasNormal=1..}] add stealTarget2
