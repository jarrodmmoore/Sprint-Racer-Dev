...
execute as @e[tag=activeplayer,tag=playerCyan,tag=!finished,tag=!self,tag=!eliminated,scores={value=1..}] run function sprint_racer:items/item_tick/normal/anvil_of_justice/check_for_lowest_hp

#target random players with the lowest health we just found
execute as @e[tag=activeplayer,tag=playerCyan,sort=random,tag=!finished,tag=!self,tag=!eliminated,scores={value=1..}] if score @s value <= #lowest value run function sprint_racer:items/item_tick/normal/anvil_of_justice/check_for_lowest_hp_go
