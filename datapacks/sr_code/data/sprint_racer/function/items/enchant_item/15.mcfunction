...
execute if entity @e[tag=w,x=1560,y=150,z=406,distance=..1,tag=banEnchant15] run return \
    run function sprint_racer:items/enchant_item/recursive_give_one {function:"sprint_racer_language:gameplay/give_item/no15"}
#=====

#this will loop up to 10 times via function recursion
clear @s chicken_spawn_egg[custom_data~{normalitem:1b}] 1
clear @s bricks[custom_data~{normalitem:1b}] 1
scoreboard players add @s enchantQty 1
execute if entity @s[scores={enchantQty=..9}] if items entity @s weapon.mainhand *[custom_data~{no15:1b}] run function sprint_racer:items/enchant_item/15
