...
execute unless score #gameState value matches 7..8 run scoreboard players remove @s moveSpeed 100

#summon text_display for damage number
execute unless block ~ -64 ~ wither_rose run summon text_display ~ ~2.2 ~ {alignment:center,text:'{"text":"-5","color":"red","bold":true}',Tags:["setscore","health_text"],billboard:"center",teleport_duration:20,shadow:1b,see_through:1b,brightness:{sky:15,block:15},transformation:{left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f],translation:[0f,0f,0f],scale:[2f,2f,2f]},background:0}
