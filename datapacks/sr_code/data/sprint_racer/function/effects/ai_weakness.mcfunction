...
execute if score global gameState matches 3 if entity @s[tag=!aiStronk,scores={aiHoldingItem=..0,aiBattleBasherDurability=1..}] run tag @s add aiStronk
execute if score global gameState matches 3 unless score #battleBatBanned value matches 1.. if entity @s[tag=!aiStronk,scores={aiHoldingItem=..0}] run tag @s add aiStronk
