tag @e[tag=aiTarget4,tag=secondaryTarget] remove secondaryTarget

#target must be an aec!
execute if entity @e[tag=aiTarget4,type=!area_effect_cloud] run function sprint_racer:ai/general/clear_old_ai_targets

#no aec target? spawn one
execute unless entity @e[tag=aiTarget4,type=area_effect_cloud] run summon area_effect_cloud ~ ~ ~ {Tags:["aiTarget4"],custom_particle:{type:"block",block_state:{Name:"air"}},Duration:6,WaitTime:0}
...
data merge entity @e[limit=1,tag=aiTarget4,type=area_effect_cloud] {custom_particle:{type:"block",block_state:{Name:"air"}},Duration:6,WaitTime:0}

#teleport aec depending what direction we're going
execute if entity @s[tag=!inWater] at @s rotated ~ 0 positioned ^ ^ ^5 run tp @e[limit=1,tag=aiTarget4,type=area_effect_cloud] ~ ~ ~
execute if entity @s[tag=inWater,scores={math2=1}] at @s rotated ~ 10 positioned ^ ^ ^5 run tp @e[limit=1,tag=aiTarget4,type=area_effect_cloud] ~ ~ ~
execute if entity @s[tag=inWater,scores={math2=2}] at @s rotated ~ 0 positioned ^ ^ ^5 run tp @e[limit=1,tag=aiTarget4,type=area_effect_cloud] ~ ~ ~
execute if entity @s[tag=inWater,scores={math2=3}] at @s rotated ~ -10 positioned ^ ^ ^5 run tp @e[limit=1,tag=aiTarget4,type=area_effect_cloud] ~ ~ ~
