...
tag @e[tag=aiTarget3,tag=!secondaryTarget,tag=!subTarget,distance=..2,tag=!requireGround,type=marker] add gotcha
execute if entity @s[nbt={OnGround:1b}] run tag @e[tag=aiTarget3,limit=1,tag=!subTarget,distance=..2,tag=requireGround,type=marker] add gotcha
