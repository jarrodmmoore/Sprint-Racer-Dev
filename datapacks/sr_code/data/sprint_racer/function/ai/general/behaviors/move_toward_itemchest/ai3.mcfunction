...
execute if entity @s[scores={itemCooldown=..0}] unless entity @e[tag=aiTarget3,tag=secondaryTarget] if entity @e[tag=itemcontainer,type=armor_stand,distance=..6,tag=!chest10,tag=!aiCantSee] run tag @e[tag=itemcontainer,type=armor_stand,limit=1,distance=..6,sort=random,tag=!chest10,tag=!aiCantSee] add newTarget
...
tag @e[tag=newTarget] add aiTarget3
tag @e[tag=newTarget] add secondaryTarget
tag @e[tag=newTarget] remove newTarget

#no chests around? stop
execute if entity @s[scores={itemCooldown=..0}] unless entity @e[tag=aiTarget3,tag=secondaryTarget] unless entity @e[tag=itemcontainer,type=armor_stand,distance=..6] run tag @s add time2stop
...
execute if score @s aiBehavior matches 1 run function sprint_racer:ai/general/behaviors/follow_breadcrumb/parallel_check_for_target_3

tag @s[scores={itemCooldown=1..}] add time2stop

execute if entity @s[tag=time2stop] run tag @e[tag=aiTarget3,tag=secondaryTarget] remove aiTarget3
