...
execute unless entity @e[tag=aiTarget1,limit=1,sort=nearest,type=marker] run function sprint_racer:ai/general/behaviors/follow_breadcrumb/create_new_target/target_nearest_instead

#reached the target, let's find a new one
tag @e[tag=aiTarget1,tag=!subTarget,distance=..2,tag=!requireGround,type=marker] add gotcha
