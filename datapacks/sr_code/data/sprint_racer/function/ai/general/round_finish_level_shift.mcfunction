...
execute if entity @e[type=armor_stand,tag=AImaster] run tag @s add updatePoints
...
execute as @e[tag=AImaster,scores={finishPos=1..}] if score @s finishPos > #math value run scoreboard players operation #math value = @s finishPos
