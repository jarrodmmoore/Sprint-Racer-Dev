...
execute if entity @s[tag=good2throw,tag=leadprojectile] as @e[limit=1,tag=projectTarget] at @s anchored eyes run summon marker ~ ~ ~ {Tags:["projectedtarget"]}
...
execute if entity @s[tag=good2throw] run function sprint_racer:ai/general/item_logic/2/use

#if a shot did NOT happen, keep holding the item and maybe try checking again soon
scoreboard players set @s[tag=!good2throw,scores={aiHasItem2=1..}] aiHoldingItem 2
scoreboard players set @s[tag=!good2throw,scores={aiHasItem2=1..}] aiActICooldown 10
...
scoreboard players set @s[scores={aiHasItem2=..0}] aiHoldingItem 0