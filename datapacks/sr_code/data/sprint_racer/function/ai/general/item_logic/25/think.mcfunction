...
execute if entity @s[tag=eatit] run function sprint_racer:ai/general/item_logic/25/use

tag @s[tag=eatit] remove eatit

scoreboard players set @s[scores={aiHoldingItem=25,hitboxHP=1020..}] aiHoldingItem 0

scoreboard players set @s[scores={aiHasItem25=..0}] aiHoldingItem 0