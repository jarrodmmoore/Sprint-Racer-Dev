...
execute as @e[tag=lookinghere,limit=1,distance=..1] store result score @e[tag=self,limit=1,distance=..1] mvmt_coord_dx run data get entity @s Pos[0] 10000
execute as @e[tag=lookinghere,limit=1,distance=..1] store result score @e[tag=self,limit=1,distance=..1] mvmt_coord_dy run data get entity @s Pos[1] 100000
execute as @e[tag=lookinghere,limit=1,distance=..1] store result score @e[tag=self,limit=1,distance=..1] mvmt_coord_dz run data get entity @s Pos[2] 10000
...
execute facing entity @e[tag=lookinghere,limit=1,distance=..1] feet positioned ^ ^ ^1 run summon snowball ~ ~1.8 ~ {Tags:["newsnowball","sHasID"]}
execute at @e[tag=newsnowball,distance=..4,type=snowball,limit=1] run playsound minecraft:entity.snowball.throw master @a ~ ~ ~ .75 .5
execute store result entity @e[limit=1,tag=newsnowball,distance=..4,type=snowball] Motion[0] double 0.0156 run scoreboard players get @s mvmt_coord_dx
execute store result entity @e[limit=1,tag=newsnowball,distance=..4,type=snowball] Motion[1] double 0.0156 run scoreboard players get @s mvmt_coord_dy
execute store result entity @e[limit=1,tag=newsnowball,distance=..4,type=snowball] Motion[2] double 0.0156 run scoreboard players get @s mvmt_coord_dz

#scoreboard players set @s useSnowball 1

execute if entity @s[tag=playerCyan] run tag @e[tag=newsnowball,distance=..4,type=snowball,limit=1] add itemCyan
scoreboard players operation @e[tag=newsnowball,distance=..4,type=snowball,limit=1] playerID = @s playerID
tag @e[tag=newsnowball,distance=..4,type=snowball,limit=1] remove newsnowball
