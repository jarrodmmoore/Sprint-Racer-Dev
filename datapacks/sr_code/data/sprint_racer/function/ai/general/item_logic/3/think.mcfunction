...
execute if entity @s[tag=good2throw] facing entity @e[tag=looktarget,limit=1,sort=nearest] eyes run function sprint_racer:ai/general/item_logic/3/use
execute if entity @s[tag=!good2throw] unless entity @e[tag=w,x=1560,y=150,z=406,distance=..1,type=armor_stand,scores={gameState=3}] run function sprint_racer:ai/general/item_logic/3/think_alt_race
execute if entity @s[tag=!good2throw] if entity @e[tag=w,x=1560,y=150,z=406,distance=..1,type=armor_stand,scores={gameState=3}] run function sprint_racer:ai/general/item_logic/3/think_alt_battle

#if a throw did NOT happen, keep holding the item and maybe try checking again soon
scoreboard players set @s[tag=!good2throw,scores={aiHasItem3=1..}] aiHoldingItem 3
scoreboard players set @s[tag=!good2throw,scores={aiHasItem3=1..}] aiActICooldown 10
...
scoreboard players set @s[scores={aiHasItem3=..0}] aiHoldingItem 0