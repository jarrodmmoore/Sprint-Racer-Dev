...
execute if entity @e[tag=w,type=armor_stand,tag=autobalance] unless entity @e[tag=w,type=armor_stand,scores={forcePlayerTeam=1..2}] run function sprint_racer:teams/auto_balance

#hidden feature: all players can be forced onto a team together
#might use this in a mission/adventure mode one day? ;)
execute if entity @e[tag=w,type=armor_stand,scores={forcePlayerTeam=1}] as @a[tag=playing] run function sprint_racer:teams/join_orange
execute if entity @e[tag=w,type=armor_stand,scores={forcePlayerTeam=2}] as @a[tag=playing] run function sprint_racer:teams/join_cyan