execute if entity @a[team=!player,tag=eliminated] unless entity @a[team=!player,tag=eliminated,scores={revengeMine=1}] run tag @e[tag=w,type=armor_stand] add resetRev
execute if entity @e[tag=w,tag=resetRev] run scoreboard objectives remove revengeMine
execute if entity @e[tag=w,tag=resetRev] run scoreboard objectives add revengeMine dummy
scoreboard players set @e[tag=w,tag=resetRev] revengeMine 1
