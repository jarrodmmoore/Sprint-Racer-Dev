...
scoreboard players set @e[tag=w,scores={difficultyRamp=..0}] maturity 100
...
execute if entity @e[tag=w,scores={maturity=3..}] run tag @e[tag=trackStandR,tag=!rtBlacklist,tag=trackIntermed] add pickViable
execute if entity @e[tag=w,scores={maturity=4..}] run tag @e[tag=trackStandR,tag=!rtBlacklist,tag=trackExpert] add pickViable
execute if entity @e[tag=w,scores={maturity=8..}] run tag @e[tag=trackStandR,tag=!rtBlacklist,tag=trackMaster] add pickViable
...
execute as @e[tag=w,type=armor_stand,tag=optInOrder,limit=1,tag=!grandprix] if entity @e[type=armor_stand,tag=!rtBlacklist,tag=trackStandR,limit=1] run function sprint_racer:random/in_order_race
