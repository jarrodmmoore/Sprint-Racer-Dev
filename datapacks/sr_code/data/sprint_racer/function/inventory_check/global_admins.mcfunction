tag @a[tag=admin,tag=!needAdminControl] add needAdminControl
tag @a[tag=!admin,tag=needAdminControl] remove needAdminControl
...
execute if entity @e[type=armor_stand,tag=w,x=1560,y=150,z=406,distance=..1,scores={gameState=1..3}] run tag @a add needVoteControl
