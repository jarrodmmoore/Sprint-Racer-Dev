...
execute if entity @e[tag=w,type=armor_stand,scores={gameTime=200..}] as @e[tag=activeplayer] if score @s lapFake > #pdaLap value run function sprint_racer:levels/praia_de_abril_2/open_next_route
...
execute if entity @e[tag=w,type=armor_stand,tag=switchON] run scoreboard players set #pdaSwitch value 1
