...
execute if score global gameState matches 1..3 if entity @e[tag=w,type=armor_stand,limit=1,tag=halftick] run function sprint_racer:levels/metro_plaza/cars_main

#alarm when players reach the 2nd floor of the bank
execute if entity @a[x=3,y=93,z=2033,dx=13,dy=5,dz=13,gamemode=!spectator] if entity @e[tag=w,scores={oTimerGlobal=0}] positioned 9 95 2042 run playsound minecraft:entity.experience_orb.pickup master @a ~ ~ ~ 2 1.15
