...
execute if entity @e[tag=w,type=armor_stand,tag=noRubberBand] run scoreboard players set @e[tag=ai,limit=9] aiRubberBand 0
...
scoreboard players add @e[tag=w,type=armor_stand,tag=halftick] GTplatTimer 1
scoreboard players set @e[tag=w,type=armor_stand,scores={GTplatTimer=49..}] GTplatTimer 1
execute if entity @e[tag=w,type=armor_stand,scores={GTplatTimer=20..21}] run effect give @a[gamemode=adventure,scores={coord_y=..310000}] minecraft:nausea 5 1 true
