...
tellraw @s ["",{text:"[restart lobby]",color:"#009988",bold:false,click_event:{action:"suggest_command",command:"/execute if entity @e[tag=w,x=1560,y=150,z=406,distance=..1,scores={gameState=0}] run function sprint_racer:game_logic/0/_initialize"},hover_event:{action:"show_text",value:"set to 5s"}}]
...
tellraw @s ["",{text:"[set custom round sequence]",color:"#009988",bold:false,click_event:{action:"suggest_command",command:"/data modify storage sprint_racer:round_sequence custom set value [1,1,1,1,2]"},hover_event:{action:"show_text",value:"set custom round sequence (1=race, 2=battle, 3=random, 4=choose)"}}]
