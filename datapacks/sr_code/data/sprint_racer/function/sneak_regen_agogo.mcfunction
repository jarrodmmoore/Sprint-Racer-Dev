...
#apply heals
execute unless entity @e[tag=52a,tag=cheats,type=armor_stand] run effect give @s minecraft:instant_health 1 0 true

#cheat code?
execute if entity @e[tag=24a,tag=cheats,type=armor_stand] if items entity @s weapon.mainhand *[custom_data~{normalitem:1b}] run function sprint_racer:items/crouch_enchant
