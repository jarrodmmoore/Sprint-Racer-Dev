execute if entity @e[tag=w,type=armor_stand,tag=absorpAlways] if score global playerRequire matches ..4 run effect give @a[tag=playing] minecraft:absorption 1000000 0 true
...
execute if entity @e[tag=w,type=armor_stand,tag=absorpAlways] if score global playerRequire matches ..4 run tag @a[tag=playing] add absorption