...
$execute if score #test4 value matches 1 positioned $(checkpoint_x) $(checkpoint_y) $(checkpoint_z) as @e[type=interaction,tag=playerPosMarker,sort=nearest] if score @s lap >= #relevantLapMax value if score @s check >= #posCalcCheck value run function sprint_racer:game_logic/1/position_calc/improved/get_pos_marker
