...
scoreboard players set @e[tag=w,type=armor_stand] specJoinTime 700
...
execute if entity @e[tag=w,type=armor_stand,tag=randomPresetA] run scoreboard players operation @e[tag=w,type=armor_stand] gamemodePresetA = @e[limit=1,sort=random,tag=random,scores={rNumber=1..3}] rNumber
...
tag @e[tag=w,type=armor_stand,tag=autotick,scores={playerCount=..5}] remove halftick
tag @e[tag=w,type=armor_stand,tag=autotick,scores={playerCount=6..}] add halftick

scoreboard players reset @e[tag=w,type=armor_stand] storm
weather clear

execute if entity @e[tag=w,type=armor_stand,tag=needAnnounce] as @e[tag=chosenTrack] run function sprint_racer:game_logic/0/vote_preview/race/v_r_index_announce
execute if entity @e[tag=w,type=armor_stand,tag=needAnnounce] as @a[tag=playing] at @s run playsound minecraft:block.note_block.bell master @s ~ 100000 ~ 100000 2
...
scoreboard players set @e[tag=w,type=armor_stand] gameState 1
scoreboard players set @e[tag=w,type=armor_stand] gameTime -50
scoreboard players set @e[tag=w,type=armor_stand] currentTimeMin 0
scoreboard players set @e[tag=w,type=armor_stand] currentTimeSec 0
scoreboard players set @e[tag=w,type=armor_stand] currentTime 0
scoreboard players set @e[tag=w,type=armor_stand] currentTimeMsec 0
scoreboard players set @e[tag=w,type=armor_stand] oTimer 0
scoreboard players set @e[tag=w,type=armor_stand] finishPos 1
scoreboard players set @e[tag=w,type=armor_stand] checkCalc 0
scoreboard players set @e[tag=w,type=armor_stand] lapCalc 0
...
execute unless entity @e[tag=w,type=armor_stand,scores={gamemodePresetA=2}] run team modify playerFinished color yellow
execute if entity @e[tag=w,type=armor_stand,scores={gamemodePresetA=2}] run team modify playerFinished color dark_gray

#TEAMS
execute if entity @e[tag=w,type=armor_stand,tag=teamplay] run function sprint_racer:teams/assemble_teams
...
tag @e[tag=w,type=armor_stand] remove ignoreCalc
tag @e[tag=w,type=armor_stand] remove calcStart
tag @e[tag=w,type=armor_stand] remove calcRunning
...
scoreboard players set @e[tag=w,type=armor_stand] timeOut 0
...
execute as @a[tag=playing] run scoreboard players add @e[tag=w,type=armor_stand] playerCountB 1
scoreboard players operation @e[tag=w,type=armor_stand] addPoints = @e[tag=w,type=armor_stand,limit=1] playerCountB
...
tag @e[tag=w,type=armor_stand] remove initFailed
...
execute if entity @e[tag=w,type=armor_stand,tag=showroundnum] run function sprint_racer:game_logic/0/sidebar/__index_main
tag @e[tag=w,type=armor_stand,tag=showroundnum] remove showroundnum
...
#scoreboard players set @e[tag=w,type=armor_stand,tag=RAIautocount] optRAIcount 4
...
scoreboard players set @e[tag=w,type=armor_stand] timeRemaining 5009
gamerule doDaylightCycle false
gamerule fallDamage true
tag @a remove darkHUD
tag @a remove eliminated
tag @e[tag=w,type=armor_stand] remove hideLaps
tag @e[tag=w,type=armor_stand] remove noCharity
tag @e[tag=w,type=armor_stand] remove gauntletTrack
...
scoreboard players set @e[tag=w,type=armor_stand,scores={gameState=1,gamemodePresetA=2}] lap 3
scoreboard players set @e[tag=w,type=armor_stand] timeUntilElim 800
execute if entity @e[tag=w,type=armor_stand,scores={gamemodePresetA=2}] run bossbar set minecraft:timeremaining max 800
scoreboard players set @e[tag=w,type=armor_stand] addPoints2 1
...
scoreboard players operation global playerRequire += @e[tag=w,type=armor_stand,limit=1] playerCountB
scoreboard players operation global playerCountM = global playerRequire
execute if entity @e[tag=w,type=armor_stand,tag=noExtraChest] run scoreboard players set global playerRequire -9999

#initialize bossbar lap display based on the set amt of laps
execute if entity @e[tag=w,type=armor_stand,tag=!classicHUD] run function sprint_racer:game_logic/1/lap_bossbar/use_custom_hud
scoreboard players set #hudMode value 1
execute if entity @e[tag=w,type=armor_stand,scores={gamemodePresetA=2}] run scoreboard players set #hudMode value 2
execute unless entity @e[tag=w,type=armor_stand,scores={gamemodePresetA=2}] run function sprint_racer:game_logic/1/lap_bossbar/setup
...
execute unless entity @e[tag=chosenTrack,tag=trackStandR] run tag @e[tag=w,type=armor_stand] add initFailed

execute unless entity @e[tag=w,type=armor_stand,tag=initFailed] run effect give @a blindness 30 1 true
execute unless entity @e[tag=w,type=armor_stand,tag=initFailed] run effect give @a slow_falling 7 1 true
execute unless entity @e[tag=w,type=armor_stand,tag=initFailed] run effect give @a invisibility 30 1 true
execute unless entity @e[tag=w,type=armor_stand,tag=initFailed] run effect give @a resistance 30 255 true
execute unless entity @e[tag=w,type=armor_stand,tag=initFailed] run effect give @a weakness 30 20 true
execute unless entity @e[tag=w,type=armor_stand,tag=initFailed] run gamemode spectator @a

#failed to load a valid track? back to lobby
execute if entity @e[tag=w,type=armor_stand,tag=initFailed] run tag @e[tag=w,type=armor_stand] add noskip2next
execute if entity @e[tag=w,type=armor_stand,tag=initFailed] run function sprint_racer:game_logic/0/_initialize

#test for lone player, send to time attack if alone
execute unless entity @e[tag=w,type=armor_stand,tag=initFailed] unless entity @e[tag=w,type=armor_stand,tag=customTesting] run schedule function sprint_racer:game_logic/1/check_for_lone_player 1t