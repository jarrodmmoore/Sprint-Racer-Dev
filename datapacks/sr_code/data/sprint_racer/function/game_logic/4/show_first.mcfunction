...
execute as @e[type=armor_stand,tag=random,scores={finishPos=1,aiPoints=1..}] run function sprint_racer_language:_dlc_3/gameplay/ai_points/announce_first
execute as @a[tag=dasMe] run function sprint_racer_language:_dlc_3/lobby/award_ceremony/give_trophy_1st

execute if entity @a[tag=dasMe] if score grandprix gameState matches 1 if entity @e[tag=w,type=armor_stand,tag=!noCrowdSFX,limit=1] as @a at @s run function sprint_racer:grand_prix/crowd_cheer_big
