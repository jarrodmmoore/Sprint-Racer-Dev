...
execute as @e[type=armor_stand,tag=random,x=1548,y=155,z=406,distance=..1,scores={finishPos=1,aiPoints=1..}] positioned 1653 82 406 run function sprint_racer:game_logic/4/spawn_ai_entity {tag:"1st_ai",model:"sr/item/sr_trophy_1st"}
execute as @e[tag=1st_ai] positioned 1653 92 406 facing 1636 88 406 run function sprint_racer:varied_teleport_thin
...
execute unless entity @a[limit=1,sort=random,tag=dasMe] as @e[limit=1,tag=1st_ai] at @s run summon firework_rocket ~ ~1 ~ {LifeTime:25,FireworksItem:{id:"firework_rocket",count:1,components:{"minecraft:fireworks":{flight_duration:2,explosions:[{shape:"star",colors:[I;14602026,15790320],fade_colors:[I;2651799,15435844,15790320],has_trail:true,has_twinkle:true},{shape:"large_ball",colors:[I;14602026,6719955,15790320],fade_colors:[I;15790320],has_trail:false,has_twinkle:false}]}}}}
...
execute if entity @a[tag=dasMe] if score grandprix gameState matches 1 if entity @e[type=armor_stand,tag=w,x=1560,y=150,z=406,distance=..1,tag=!noCrowdSFX,limit=1] as @a at @s run function sprint_racer:grand_prix/crowd_cheer_big
 
execute unless entity @a[tag=dasMe] unless entity @e[limit=1,tag=1st_ai] run function sprint_racer_language:lobby/award_ceremony/nobody_first
execute unless entity @a[tag=dasMe] unless entity @e[limit=1,tag=1st_ai] run particle large_smoke 1653 93 406 0.5 0.5 0.5 0 20 force
execute unless entity @a[tag=dasMe] unless entity @e[limit=1,tag=1st_ai] run playsound minecraft:entity.villager.no master @a 1653 93 406 3 1.2
