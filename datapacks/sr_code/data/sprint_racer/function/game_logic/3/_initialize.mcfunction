...
execute if entity @e[tag=w,type=armor_stand,tag=randomPresetB] run scoreboard players operation @e[tag=w,type=armor_stand] gamemodePresetB = @e[limit=1,sort=random,tag=random,scores={rNumber=1..3}] rNumber
...
execute if entity @e[tag=w,type=armor_stand,tag=needAnnounce] as @e[tag=chosenTrack] run function sprint_racer:game_logic/0/vote_preview/battle/v_b_index_announce
...
execute if entity @e[tag=w,type=armor_stand,scores={gamemodePresetB=3}] run function sprint_racer:items/item_presets/capsules_only

#boiler-plate stuff for starting a battle
scoreboard players set @e[tag=w,type=armor_stand] gameState 3
...
#tag @e[tag=w,type=armor_stand] add allowPVP
...
execute unless entity @e[tag=w,type=armor_stand,scores={gamemodePresetB=2}] run team modify playerFinished color yellow
execute if entity @e[tag=w,type=armor_stand,scores={gamemodePresetB=2}] run team modify playerFinished color dark_gray
...
tag @e[tag=w,type=armor_stand] remove suddenDeath
...
execute if entity @e[tag=w,type=armor_stand,scores={gamemodePresetB=2}] run scoreboard players set #hudMode value 5
...
scoreboard players set @e[tag=w,type=armor_stand,scores={gamemodePresetB=2}] timeRemaining 7408
scoreboard players set @e[tag=w,type=armor_stand,scores={gamemodePresetB=2}] countTimeMin 6
scoreboard players set @e[tag=w,type=armor_stand] countTimeSec 0
scoreboard players set @e[tag=w,type=armor_stand] countTimeMsec 9
bossbar set timeremaining max 4800
execute if entity @e[tag=w,type=armor_stand,scores={gamemodePresetB=2}] run bossbar set timeremaining max 7200
...
#execute if entity @e[tag=w,type=armor_stand,tag=realms,tag=optBAIalways,tag=!noAItrack] run function sprint_racer:game_logic/3/ai_count_reduce

#======================================

#if no chosen track exists, this function failed and we'll go back to the lobby (game logic 0)
execute unless entity @e[tag=chosenTrack,tag=trackStandB] run tag @e[tag=w,type=armor_stand] add initFailed
...
execute unless entity @e[tag=w,type=armor_stand,tag=initFailed] unless entity @e[tag=w,type=armor_stand,tag=customTesting] run schedule function sprint_racer:game_logic/3/check_for_lone_player 1t