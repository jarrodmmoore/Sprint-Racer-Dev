execute if entity @s[tag=AImaster,type=armor_stand,x=1548,y=155,z=406,distance=..1] run scoreboard players operation #getPlayer value = @s playerID
execute if entity @s[tag=AImaster,type=armor_stand,x=1548,y=155,z=406,distance=..1] as @e[tag=ai] if score @s playerID = #getPlayer value run tag @s add show1UP
...
execute if entity @s[tag=AImaster,type=armor_stand,x=1548,y=155,z=406,distance=..1] as @e[tag=show1UP] at @s if loaded ~ ~ ~ run summon text_display ~ ~2.2 ~ {alignment:center,text:'{"text":"sr.game.1up","color":"white","bold":true}',Tags:["setscore","health_text"],billboard:"center",teleport_duration:20,shadow:1b,see_through:1b,brightness:{sky:15,block:15},transformation:{left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f],translation:[0f,0f,0f],scale:[2f,2f,2f]},background:0}
...
execute if entity @s[tag=AImaster,type=armor_stand,x=1548,y=155,z=406,distance=..1] as @e[tag=show1UP] at @s run playsound minecraft:entity.player.levelup master @a ~ ~ ~ 1.75 0.75
execute if entity @s[tag=AImaster,type=armor_stand,x=1548,y=155,z=406,distance=..1] as @e[tag=show1UP] at @s run particle heart ~ ~1 ~ 1 1 1 0 20 force
