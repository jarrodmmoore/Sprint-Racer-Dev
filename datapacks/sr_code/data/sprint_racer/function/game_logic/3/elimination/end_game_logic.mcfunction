...
execute unless entity @s[tag=allowAIfinish] unless entity @a[tag=playing,tag=!eliminated] if entity @e[type=armor_stand,tag=AImaster,x=1548,y=155,z=406,distance=..1,tag=!aiDisable] if entity @s[scores={timeRemaining=101..}] run function sprint_racer:game_logic/3/elimination/skip_to_sudden_death
