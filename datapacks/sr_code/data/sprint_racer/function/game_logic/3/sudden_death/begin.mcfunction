tag @e[type=armor_stand,tag=w,x=1560,y=150,z=406,distance=..1] add suddenDeath

scoreboard players set @a[tag=playing,tag=!eliminated] playerLives 1
scoreboard players set @e[type=armor_stand,tag=AImaster,x=1548,y=155,z=406,distance=..1,tag=!aiDisable] playerLives 1

execute as @e[type=armor_stand,tag=AImaster,x=1548,y=155,z=406,distance=..1] run function sprint_racer:game_logic/3/master_send_data_to_entity

function sprint_racer_language:_dlc_1/gameplay/sudden_death_start