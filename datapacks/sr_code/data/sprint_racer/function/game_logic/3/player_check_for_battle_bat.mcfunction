...
tag @s[nbt={Inventory:[{id:"minecraft:wooden_sword",components:{"minecraft:custom_data":{battlebat:1b}}}]}] add hasBBat
...
execute if entity @s[tag=!hasBBat,tag=!noInventory,scores={hp=1..}] unless entity @s[scores={death=1..}] unless entity @s[scores={timeSinceDeath=..10}] run function sprint_racer_language:_dlc_4/gameplay/give_battle_bat