...
scoreboard players add @e[tag=AImaster,type=armor_stand] KOs 0
...
scoreboard players set @e[tag=AImaster,type=armor_stand] battlePos 1

#clear tags
tag @a[tag=playing] remove gotRank
tag @e[tag=AImaster,type=armor_stand] remove gotRank

#don't let players with negative KOs mess with calc
tag @a[tag=players,scores={KOs=..-999999}] add gotRank
tag @e[tag=AImaster,type=armor_stand,scores={KOs=..-999999}] add gotRank
