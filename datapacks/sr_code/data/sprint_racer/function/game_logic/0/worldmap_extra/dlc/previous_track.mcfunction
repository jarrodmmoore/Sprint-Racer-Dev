...
execute if entity @e[tag=w,type=armor_stand,scores={wmDlcNum=23,wmDlcMode=1}] if entity @e[type=armor_stand,tag=cheats,tag=38] run scoreboard players set #secret_track_r value 1
execute if entity @e[tag=w,type=armor_stand,scores={wmDlcNum=6,wmDlcMode=2}] if entity @e[type=armor_stand,tag=cheats,tag=46] run scoreboard players set #secret_track_b value 1
