#scoreboard players add @e[tag=w,x=1560,y=150,z=406,distance=..1,type=armor_stand] wmCustomNum 1
...
#if this group has more than 18 unseen entries, scroll down it instead of immediately going to the next group
...
scoreboard players operation #math2 value = #trackGridPage value
scoreboard players add #math2 value 1
execute store result storage sprint_racer:func_args track_group int 1 run scoreboard players get #math2 value
execute unless score #math value matches 18.. run function sprint_racer:game_logic/0/worldmap_extra/custom/grid_layout/grouped_custom/try_next_page with storage sprint_racer:func_args
