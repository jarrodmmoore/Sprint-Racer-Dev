execute if entity @e[tag=w,tag=!choosingTrack] run scoreboard players set @a inputCooldown 150
tag @e[tag=w,tag=!choosingTrack] add choosingTrack

#reload custom track selector if something was modified in the custom track manager
execute if entity @e[tag=w,type=armor_stand,tag=wmCustomReload] if block 2480 98 266 air if block 2485 106 269 barrier if entity @e[tag=wmCustomStand] run function sprint_racer:game_logic/0/worldmap_custom_reload
