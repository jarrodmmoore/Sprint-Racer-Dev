...
execute if entity @s[tag=!need_admin] as @e[tag=w,type=armor_stand,limit=1] run function sprint_racer:game_logic/0/choose_track_return_to_lobby

#admin mode is ON, must be admin to execute
execute if entity @s[tag=need_admin] if entity @s[tag=admin] as @e[tag=w,type=armor_stand,limit=1] run function sprint_racer:game_logic/0/choose_track_return_to_lobby
