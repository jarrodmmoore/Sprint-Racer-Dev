tag @e[tag=w,tag=banNo12] add unban


tag @e[tag=w,tag=!unban] add banNo12
execute if entity @e[tag=w,tag=!unban] run tag @e[tag=random,scores={rNumber=12}] add capsuleBan
...
tag @e[tag=w,tag=unban] remove banNo12
execute if entity @e[tag=w,tag=unban] run tag @e[tag=random,scores={rNumber=12}] remove capsuleBan
