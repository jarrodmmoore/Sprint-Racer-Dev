tag @e[tag=w,tag=banNo10] add unban


tag @e[tag=w,tag=!unban] add banNo10
execute if entity @e[tag=w,tag=!unban] run tag @e[tag=random,scores={rNumber=10}] add capsuleBan
...
tag @e[tag=w,tag=unban] remove banNo10
execute if entity @e[tag=w,tag=unban] run tag @e[tag=random,scores={rNumber=10}] remove capsuleBan
