tag @e[tag=w,tag=banNo11] add unban


tag @e[tag=w,tag=!unban] add banNo11
execute if entity @e[tag=w,tag=!unban] run tag @e[tag=random,scores={rNumber=11}] add capsuleBan
...
tag @e[tag=w,tag=unban] remove banNo11
execute if entity @e[tag=w,tag=unban] run tag @e[tag=random,scores={rNumber=11}] remove capsuleBan
