tag @e[tag=w,tag=banNo18] add unban


tag @e[tag=w,tag=!unban] add banNo18
execute if entity @e[tag=w,tag=!unban] run tag @e[tag=random,scores={rNumber=18}] add capsuleBan
...
tag @e[tag=w,tag=unban] remove banNo18
execute if entity @e[tag=w,tag=unban] run tag @e[tag=random,scores={rNumber=18}] remove capsuleBan
