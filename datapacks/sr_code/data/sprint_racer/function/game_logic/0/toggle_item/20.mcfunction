tag @e[tag=w,tag=banNo20] add unban


tag @e[tag=w,tag=!unban] add banNo20
execute if entity @e[tag=w,tag=!unban] run tag @e[tag=random,scores={rNumber=20}] add capsuleBan
...
tag @e[tag=w,tag=unban] remove banNo20
execute if entity @e[tag=w,tag=unban] run tag @e[tag=random,scores={rNumber=20}] remove capsuleBan
