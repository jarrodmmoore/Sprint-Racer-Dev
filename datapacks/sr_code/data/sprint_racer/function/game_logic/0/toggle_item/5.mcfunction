tag @e[tag=w,tag=banNo5] add unban


tag @e[tag=w,tag=!unban] add banNo5
execute if entity @e[tag=w,tag=!unban] run tag @e[tag=random,scores={rNumber=5}] add capsuleBan
...
tag @e[tag=w,tag=unban] remove banNo5
execute if entity @e[tag=w,tag=unban] run tag @e[tag=random,scores={rNumber=5}] remove capsuleBan
