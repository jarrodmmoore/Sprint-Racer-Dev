...
execute as @e[type=snowball,tag=targetVisual] at @s unless entity @e[tag=taTargetCore,distance=..1] run kill @s

#alternate target visuals, these are cool but kinda laggy and jank
#execute as @a[limit=5,sort=random,gamemode=!creative] at @s positioned ^ ^ ^8.8 at @e[limit=3,sort=nearest,tag=taTargetCore] positioned ~ ~.5 ~ facing entity @s eyes run function sprint_racer:game_logic/8/targets/particles
#execute as @a[limit=5,sort=random,gamemode=!creative] at @s at @e[limit=3,sort=nearest,tag=taTargetCore,distance=..10] positioned ~ ~.5 ~ facing entity @s eyes run function sprint_racer:game_logic/8/targets/particles_nearby
...
execute as @e[tag=missile] at @s if entity @e[tag=taTargetCore,distance=..2] run function sprint_racer:items/item_tick/missile_explode
execute as @e[type=snowball,tag=!targetVisual] at @s run tag @e[tag=taTargetCore,distance=..4] add targetHit
...
execute if entity @s[scores={gameTime=165..}] as @e[tag=taTargetCore] at @s run function sprint_racer:game_logic/8/targets/hit_detect