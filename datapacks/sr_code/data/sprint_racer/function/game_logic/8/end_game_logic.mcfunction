...
execute unless entity @a[tag=playing,scores={playerState=2}] if entity @e[tag=w,type=armor_stand,scores={timeRemaining=0..1220}] run function sprint_racer:game_logic/8/race_end_timer

#no unfinished players remain, start the end sequence (60 ticks with no active players)
scoreboard players add @s timeOut 1
execute if entity @a[tag=playing,limit=1] run scoreboard players set @e[tag=w,type=armor_stand] timeOut 0
