...
execute if entity @e[tag=w,x=1560,y=150,z=406,distance=..1,type=armor_stand,tag=grandprix] as @a run scoreboard players operation @s dummyPoints = @s gpPoints

#now add the points into the saved objectives (must be done immediately to avoid re-joined players losing their gp points)
execute if entity @e[tag=w,x=1560,y=150,z=406,distance=..1,type=armor_stand,tag=endlessMode,tag=!grandprix] as @a run scoreboard players operation @s endlessPoints += @s addPoints
execute if entity @e[tag=w,x=1560,y=150,z=406,distance=..1,type=armor_stand,tag=grandprix] as @a run scoreboard players operation @s gpPoints += @s addPoints